MASTER_NODE='mariadb-001'
SLAVE01_NODE='mariadb-002'
SLAVE02_NODE='mariadb-003'

EXEC_MASTER="sudo docker exec ${MASTER_NODE} mysql -uroot -proot -N -e "
EXEC_SLAVE01="sudo docker exec ${SLAVE01_NODE} mysql -uroot -proot -e "
EXEC_SLAVE02="sudo docker exec ${SLAVE02_NODE} mysql -uroot -proot -e "
...
sudo docker exec ${MASTER_NODE} sh /opt/exporters/node_exporter/start_node_exporter.sh
sudo docker exec ${MASTER_NODE} sh /opt/exporters/mysqld_exporter/start_mysqld_exporter.sh
sudo docker exec ${SLAVE01_NODE} sh /opt/exporters/node_exporter/start_node_exporter.sh
...
${EXEC_PROXY} "INSERT INTO mysql_servers(hostgroup_id, hostname, port) VALUES (10, 'mariadb-001', 3306)" 2>&1 | grep -v "Using a password"
${EXEC_PROXY} "INSERT INTO mysql_servers(hostgroup_id, hostname, port) VALUES (20, 'mariadb-001', 3306)" 2>&1 | grep -v "Using a password"
${EXEC_PROXY} "INSERT INTO mysql_servers(hostgroup_id, hostname, port) VALUES (20, 'mariadb-002', 3306)" 2>&1 | grep -v "Using a password"
${EXEC_PROXY} "INSERT INTO mysql_servers(hostgroup_id, hostname, port) VALUES (20, 'mariadb-003', 3306)" 2>&1 | grep -v "Using a password"
